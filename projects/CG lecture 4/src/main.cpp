...
bool initGLFW() {
	...
	window = glfwCreateWindow(800, 800, "Ame Gilham - Lecture 4 Hands on - INFR1350U", nullptr, nullptr);
...
int main() {
	...
	//Projection matrix - 45 degrees FoV, aspect ratio, viewing range 0.1- 100 units
	int width, height;
	...
	glm::mat4 View = glm::lookAt(
		glm::vec3(0,0,3),//3 units away from the screen
		glm::vec3(0,0,0), //looking at the origin
		glm::vec3(0,1,0) //up vector is positive on the y-axis
		); 
	...
	// T * R * S <--- from the right, scale then rotate, then translate 
	...
	while (!glfwWindowShouldClose(window)) {
		...
		Model = glm::mat4(1.0f);
		keyboard();
		Model = glm::translate(Model, glm::vec3(0.0f, 0.5f, 0.0f)); //move the top point to the origin 
		Model = glm::rotate(Model, glm::radians(rotY), glm::vec3(0.0f, 0.0f, 1.0f)); //rotate
		Model = glm::translate(Model, glm::vec3(0.0f, -0.5f, 0.0f)); //move it back to it's original pos 
