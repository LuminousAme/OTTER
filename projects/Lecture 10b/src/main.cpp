...
// Only during the initialisation
	...
	GLuint UlocTime = glGetUniformLocation(shader_program, "time");
	GLfloat time = 0.0f;
	GLfloat lastTime = glfwGetTime();

	GLuint UlocSpeed = glGetUniformLocation(shader_program, "speed");
	GLfloat speed = 5.0f;
	...
	while (!glfwWindowShouldClose(window)) {
		...
		glUniform1f(UlocTime, time);
		glUniform1f(UlocSpeed, speed);
